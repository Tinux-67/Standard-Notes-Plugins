...
- **Smart Scaling**: High-performance rendering using priority sampling and edge pruning, allowing the visualization of thousands of notes without lagging.
...
- **Interactive Tooltips**: Hover over nodes to see note details (React Portal powered).
...
- `src/types/sn-api.d.ts` - Type definitions for the SN Extension API
...
4. **Optimizes for Scale**:
   - **Edge Pruning**: Removes links from overly common tags (noise) to maintain clarity.
   - **Priority Sampling**: Uses a hub-preservation algorithm to render a representative snapshot of the most connected notes when datasets are large.
5. Renders an interactive graph using D3.js.
6. Allows filtering by search query and selected tags.
